...
void Consola::setPuerto(UINT puerto) {

}

void Consola::setRutaDescargas(std::string ruta) {
...
void Consola::pantallaPrincipal() {
	...
	do {
		clrscr();
		std::cout << std::endl << " FI-TORRENT" << std::endl
		<< " [1] Agregar Torrent" << std::endl
		<< " [2] Listar Torrents" << std::endl << " [3] Configurar"
		<< std::endl << " [4] Salir" << std::endl << "Opcion: ";
		int opcion;
		std::cin >> opcion;
		switch (opcion) {
		case 1:
...
void Consola::pantAgrTorrent() {
	...
	std::string url;
	std::cout << std::endl << " FI-TORRENT" << std::endl
	<< " [1] Ingresar URL .torrent" << std::endl << " [2] Volver"
	<< std::endl << "Opcion: ";
	int opcion;
	std::cin >> opcion;
	if (opcion == 1) {
		clrscr();
		std::cout << std::endl << "  FI-TORRENT" << std::endl << "  URL: ";
		std::cin >> url;
		Torrent* t = controlador->agregarTorrent(url);
		if (t != NULL) {
			addTorrent(t);
			controlador->continuarTorrent(t);
			std::cout << "  Se agrego el torrent: " << t->getNombre()
			<< std::endl;
		} else
			std::cout << "  No se pudo agregar el torrent desde " << url
			<< std::endl;
		std::cout << std::endl << " [1] Volver" << std::endl << "Opcion: ";
		std::cin >> opcion;
	}
}
...
void Consola::pantListaTorrent() {
	...
	while (continuar) {
		...
		if (cantTorrent != 0) {
			...
			while (it != end) {
				std::cout << " [" << i << "] " << (*it)->getEstado() << "   "
				<< (*it)->getNombre() << std::endl;
				it++;
				i++;
			}
		}
...
void Consola::mostrarDatosTorrent(int numTorrent) {
	std::list<Torrent*>::iterator it = torrents.begin();
	for (int i = 0; i < numTorrent; i++)
		it++;
	bool volver = false;
	bool borrar = false;
	while (!volver) {
		clrscr();
		std::cout << "\n DATOS TORRENT" << std::endl << " NOMBRE: "
		<< (*it)->getNombre() << std::endl << " ESTADO: "
		<< (*it)->getEstado() << std::endl << " DESCARGADO: "
		<< (*it)->bytesToString((*it)->getTamanioDescargado())
		<< " SUBIDO: " << (*it)->bytesToString(
				(*it)->getTamanioSubido()) << std::endl << " PROGRESO: "
				<< (int) ((((*it)->getTamanioDescargado() / 1024) * 100)
						/ ((*it)->getTamanio() / 1024)) << " %" << std::endl
						<< " CANTIDAD PEERS: " << (*it)->getCantPeers() << std::endl
						<< " CANTIDAD ARCHIVOS: "
						<< (*it)->getFileManager()->getCantArchivos() << std::endl;
		std::cout << std::endl << " [1] VOLVER" << std::endl
		<< " [2] DETENER/CONTINUAR" << std::endl
		<< " [3] BORRAR (el torrent se borra luego de elegir volver.)"
		<< std::endl << " [4] REFRESCAR PEERS" << std::endl
		<< "Opcion: ";
		int opcion;
		std::cin >> opcion;
		switch (opcion) {
